...
export const Card = React.memo(({ card, index, hovered, setHovered }: { card: any; index: number; hovered: number | null; setHovered: React.Dispatch<React.SetStateAction<number | null>> }) => (
	<div
		...
		className={cn(
			"rounded-lg relative bg-gray-900 dark:bg-neutral-900 overflow-hidden h-60 md:h-64 w-full transition-all duration-300 ease-out  ",
			hovered !== null && hovered !== index && "blur-sm scale-[0.98]"
		)}
	>
		<Image src={card.src} alt={card.name} fill className="object-cover" />
		...
		<div className={cn("absolute inset-0 bg-black/50 flex items-end py-8 px-4 transition-opacity duration-300", hovered === index ? "opacity-100" : "opacity-0")}>
			<div className="text-white text-left ">
				<h1 className="text-xl md:text-2xl font-medium bg-clip-text text-transparent bg-gradient-to-b from-neutral-50 to-neutral-200">{card.name}</h1>
				<p>
					Placed at: {card.company} <span>package: {card.package}</span>
				</p>
				<p>Designation {card.designation}</p>
				<p>
					Course: {card.course} <span>Batch: {card.batch}</span>
				</p>
			</div>
		</div>
	</div>
